...
function usage() {
    print "Global Option Variables:" > STDERR
    print "   -v nohead=BOOLEAN       Do not print a header on line 1 if true" > STDERR
    print "   -v show=id              Show the id parameter for fast and slow methods" > STDERR
    print "   -v show=when            Show local timestamps for the fast and slow methods" > STDERR
    print "   -v histogram=calls      Show histogram for api calls over time" > STDERR
    print "   -v histogram=STRING     Show histogram for api called 'method'/string over time" > STDERR
    print "   -v population=BOOLEAN   Use population standard deviation if true" > STDERR
    print "   -v resolution=minutes   Set time difference resolution to tens of minutes" > STDERR
    print > STDERR
    print "Input: RFC 4180 compliant CSV file with CRLF newlines generated by 'gcloud logging read'" > STDERR
    print "   with at least the following fields:" > STDERR
    print "      latency" > STDERR
    print "      timestamp" > STDERR
    print "      request_url" > STDERR
    print > STDERR
    print "Output: TSV (tab separated values) file with LF newlines with a histogram or call summary" > STDERR
    print "      x̄ = sample mean (average)" > STDERR
    print "      s = sample standard deviation" > STDERR
    print "      μ = population mean (average)" > STDERR
    print "      σ = population standard deviation" > STDERR
    print > STDERR
    print "Example Google Cloud SDK command:" > STDERR
    print "   gcloud logging read 'httpRequest.userAgent:\"menu/\" AND (" > STDERR
    print "      (timestamp >= \\\"2021-02-01T11:00:00+09\\\"" > STDERR
    print "         AND timestamp < \\\"2021-02-01T14:00:00+09\\\") OR" > STDERR
    print "      (timestamp >= \\\"2021-02-01T17:00:00+09\\\"" > STDERR
    print "          AND timestamp < \\\"2021-02-01T22:00:00+09\\\")" > STDERR
    print "   )' --format=\"csv(httpRequest.latency,timestamp,httpRequest.requestUrl)\"" > STDERR
}
...
$3 ~ /[?&]method=[[:alpha:]]/ {
    ...
    s = match($field["request_url"], /[?&_]id=[[:digit:]]+$/)           # extract id from last "&id=" parameter
